...
import net.minidev.json.parser.ParseException;//issacyxw
...
public class FVClassifier implements FVEventHandler, FVSendMsg, FlowMapChangedListener, FlowvisorChangedListener,
	...
	public ConcurrentHashMap<String, Integer> weightMap= new ConcurrentHashMap<String, Integer>();//构造函数内初始化weightMap
	...
	public void init() throws IOException, ParseException {
		...
		server s= new server();//issac
        s.startThread(this);//issac
...
//		System.out.println("fvclassifer  size= "+this.weightMap.size());
//		System.out.println("weightMap= "+this.weightMap);
//		System.out.println("SlicerMap= "+this.getSwitchFlowMap());
		if (this.weightMap.size()==0){
			
			for (FVSlicer fvSlicer : this.getSlicers()) {
            	//System.out.println(fvSlicer.getName());
				//if (fvSlicer.isConnected())
				this.weightMap.put(fvSlicer.getName(),1);
			}
		}
		// else{//这段需要由api调用
		// 	this.weightMap = new ConcurrentHashMap<String, Integer>();//构造函数内初始化weightMap
        ...
        //     	//System.out.println(fvSlicer.getName());
		// 		//if (fvSlicer.isConnected())
		// 		this.weightMap.put(fvSlicer.getName(),10);
		// 	}
		// }
	...
	public void sendMsg(OFMessage msg, FVSendMsg from) {
        //System.out.print("classifier send");
		if (this.msgStream != null) {
