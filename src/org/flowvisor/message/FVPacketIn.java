...
public class FVPacketIn extends OFPacketIn implements Classifiable, Slicable,
	...
	// public ConcurrentHashMap<String, Integer> weightMap;

	// public FVPacketIn(){
	// 	super();
	// 	this.weightMap = new ConcurrentHashMap<String, Integer>();//构造函数内初始化weightMap
	// }
...
//        System.out.println("Packrt in!!! slicer.size()="+fvClassifier.getSlicers().size());
		this.immByWeight(fvClassifier);
	...
	public void immByWeight(FVClassifier fvClassifier){
//		FVSlicer fvSlicer = fvClassifier.getSlicerByName(sliceAction
//				.getSliceName());
		short inport = this.getInPort();
//		this.setXid(0xdeaddead); // mark this as broadcasted

		//ConcurrentHashMap<String, Integer> weightMap = new ConcurrentHashMap<String, Integer>();//weight怎么传？api？
		// for (FVSlicer fvSlicer : fvClassifier.getSlicers()) {
        //     //System.out.println(fvSlicer.getName());
		// 	//if (fvSlicer.isConnected())
		// 	fvClassifier.weightMap.put(fvSlicer.getName(),10);
		// }
		...
		int total=0;
		int cnt=0;
		for (String sliceName : fvClassifier.weightMap.keySet()) {
			//if (fvClassifier.getSlicers().contains(sliceName)) {
//                System.out.println("sliceName="+sliceName);
				total+=fvClassifier.weightMap.get(sliceName);
                cnt+=1;
...
//                System.out.println("case1");
		...
		else if(total==0){//默认没有,按等分概率选择一个slicer发送
                        
		hash=Math.abs(hash%cnt)+1;
//        System.out.println("hash="+hash);
		int idx=0;
		// for (String sliceName : weightMap.keySet()) {
		// 		//if (fvClassifier.getSlicers().contains(sliceName)) {
		// 			idx+=1;
		// 			if(idx>=hash){
        //                 System.out.println("send to "+sliceName);
		// 				fvClassifier.getSlicerByName(sliceName).sendMsg(this,fvClassifier);
		// 				return;
		// 			}
		// 		//}
		// 	}
		for (FVSlicer fvSlicer : fvClassifier.getSlicers()) {
			//if (fvClassifier.getSlicers().contains(sliceName)) {
				idx+=1;
				if(idx>=hash){
					System.out.println("send to "+fvSlicer.getSliceName());
					System.out.println("----------------------------------------------------------------");
			...
			hash=Math.abs(hash%total)+1;
//            System.out.println("hash="+hash);
			int point=0;
			// for (String sliceName : weightMap.keySet()) {
			// 	//if (fvClassifier.getSlicers().contains(sliceName)) {
			// 		point+=weightMap.get(sliceName);
			// 		if(point>=hash){
            //             System.out.println("send to"+sliceName);
			// 			fvClassifier.getSlicerByName(sliceName).sendMsg(this,fvClassifier);
			// 			return;
			...
			for (FVSlicer fvSlicer : fvClassifier.getSlicers()) {
				//if (fvClassifier.getSlicers().contains(sliceName)) {
					point+=fvClassifier.weightMap.get(fvSlicer.getName());
					if(point>=hash){
						System.out.println("send to"+fvSlicer.getName());
						System.out.println("----------------------------------------------------------------");
						fvSlicer.setBufferIDAllowed(this.getBufferId());
						fvSlicer.sendMsg(this,fvClassifier);
						return;
